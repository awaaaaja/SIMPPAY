...
function submit() {
    form.put(route(`${prefix.value}.gaji-pokok-scale.update`, props.item.id));
}
</script>

<template>
    <Head title="Edit Gaji Pokok Scale" />

    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-semibold leading-tight text-gray-800">Edit Gaji Pokok Scale</h2>
                <Link :href="route(`${prefix}.gaji-pokok-scale.index`)" class="text-sm text-gray-600 hover:underline">Kembali</Link>
            </div>
        </template>
                ...
                <div class="bg-white p-6 shadow-sm sm:rounded-lg">
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div>
                            ...
                            <Input id="mkg" v-model="form.mkg" type="number" min="0" step="1" class="mt-1" required />
                            <p v-if="form.errors.mkg" class="mt-1 text-sm text-destructive">{{ form.errors.mkg }}</p>
                        </div>
                        ...
                        <div class="flex items-center justify-end gap-3">
                            <Link :href="route(`${prefix}.gaji-pokok-scale.index`)" class="rounded-lg border border-input bg-background px-4 py-2 text-sm font-medium hover:bg-accent hover:text-accent-foreground">Batal</Link>
