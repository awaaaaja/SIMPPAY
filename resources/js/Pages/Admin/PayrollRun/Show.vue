            ...
            <div class="flex items-center justify-between">
                <div>
                    <button @click="router.get(route(`${prefix}.payroll-run.index`))"
                        class="text-sm text-gray-500 hover:text-[#025AB1] mb-1 transition-colors">
                        Kembali
                    </button>
                    ...
                    <table class="min-w-full divide-y divide-gray-100">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            <tr v-for="detail in run.details" :key="detail.id"
                                ...
                                <td class="px-6 py-4 text-sm text-[#025AB1] text-right">+{{ formatRupiah(detail.total_tunjangan_tambahan) }}</td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <a :href="route(`${prefix}.slip-gaji.pdf`, detail.id)" target="_blank" @click.stop
                                        class="inline-flex items-center px-2 py-1 text-xs font-medium text-[#025AB1] bg-[#025AB1]/10 rounded-[6px] hover:bg-[#025AB1]/20 transition-colors">
                                        Cetak
                                    </a>
                                </td>
                            </tr>
                            ...
                            <div class="space-y-1">
                                ...
                                <div v-if="selectedDetail.breakdown_json?.tunjangan?.length" class="pt-1 border-t border-gray-50">
                                    <div v-for="(t, i) in selectedDetail.breakdown_json.tunjangan" :key="i" class="flex justify-between text-sm">
                                        ...
                                        <span class="text-[#025AB1]">{{ formatRupiah(t.nominal) }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
