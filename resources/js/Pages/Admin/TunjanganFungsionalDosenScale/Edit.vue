...
const form = useForm({
    jabatan_fungsional: props.item.jabatan_fungsional,
    angka_kredit: props.item.angka_kredit,
    pangkat: props.item.pangkat,
    golongan_ruang: props.item.golongan_ruang,
    nominal: props.item.nominal,
});

function submit() {
    form.put(route(`${prefix.value}.tunjangan-fungsional-dosen-scale.update`, props.item.id));
}
</script>

<template>
    <Head title="Edit Tunjangan Fungsional Dosen Scale" />

    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-semibold leading-tight text-gray-800">Edit Tunjangan Fungsional Dosen Scale</h2>
