...
const form = useForm({
    ...
    urutan: props.item.urutan,
});

function submit() {
    form.put(route(`${prefix.value}.golongan-ruang.update`, props.item.id));
}
</script>

<template>
    <Head title="Edit Golongan Ruang" />

    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-semibold leading-tight text-gray-800">Edit Golongan Ruang</h2>
                <Link :href="route(`${prefix}.golongan-ruang.index`)" class="text-sm text-gray-600 hover:underline">Kembali</Link>
            </div>
        </template>
                ...
                <div class="bg-white p-6 shadow-sm sm:rounded-lg">
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div class="flex items-center justify-end gap-3">
                            <Link :href="route(`${prefix}.golongan-ruang.index`)" class="rounded-lg border border-input bg-background px-4 py-2 text-sm font-medium hover:bg-accent hover:text-accent-foreground">Batal</Link>
