...
function applyFilter() {
    router.get(route(`${prefix.value}.level-struktur.index`), { search: search.value }, {
        preserveState: true,
        replace: true,
    });
}
...
function doDelete(id) {
    router.delete(route(`${prefix.value}.level-struktur.destroy`, id));
}
</script>

<template>
    <Head title="Data Level Struktur" />

    <AuthenticatedLayout>
        <div class="px-4 pt-6 pb-2 sm:px-6 lg:px-8 max-w-7xl mx-auto">
            <div class="flex items-center justify-between mb-6">
                <h1 class="text-2xl font-semibold text-gray-800">Data Level Struktur</h1>
                <Button v-if="can.create" as-child>
                    <Link :href="route(`${prefix}.level-struktur.create`)">Tambah Level Struktur</Link>
                </Button>
            </div>
                            ...
                            <TableCell class="text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <Button v-if="can.update" variant="ghost" size="icon-sm" as-child>
                                        <Link :href="route(`${prefix}.level-struktur.edit`, item.id)">
                                    ...
                                    <AlertDialog v-if="can.delete">
                                        ...
                                        <AlertDialogContent>
                                            <AlertDialogHeader>
                                                <AlertDialogTitle>Hapus Level Struktur</AlertDialogTitle>
