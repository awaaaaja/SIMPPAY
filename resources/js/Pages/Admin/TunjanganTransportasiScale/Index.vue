...
function applyFilter() {
    router.get(route(`${prefix.value}.tunjangan-transportasi-scale.index`), { search: search.value }, {
        preserveState: true,
        replace: true,
    });
}
...
function doDelete(id) {
    router.delete(route(`${prefix.value}.tunjangan-transportasi-scale.destroy`, id));
}
</script>

<template>
    <Head title="Data Tunjangan Transportasi Scale" />

    <AuthenticatedLayout>
        <div class="px-4 pt-6 pb-2 sm:px-6 lg:px-8 max-w-7xl mx-auto">
            <div class="flex items-center justify-between mb-6">
                <h1 class="text-2xl font-semibold text-gray-800">Data Tunjangan Transportasi Scale</h1>
                <Button v-if="can.create" as-child>
                    <Link :href="route(`${prefix}.tunjangan-transportasi-scale.create`)">Tambah Tunjangan Transportasi Scale</Link>
                </Button>
            </div>
                ...
                <Table>
                    ...
                    <TableBody>
                        <TableRow v-for="(item, index) in items.data" :key="item.id">
                            ...
                            <TableCell>{{ item.golongan_range }}</TableCell>
                            ...
                            <TableCell class="text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <Button v-if="can.update" variant="ghost" size="icon-sm" as-child>
                                        <Link :href="route(`${prefix}.tunjangan-transportasi-scale.edit`, item.id)">
                                    ...
                                    <AlertDialog v-if="can.delete">
                                        ...
                                        <AlertDialogContent>
                                            <AlertDialogHeader>
                                                <AlertDialogTitle>Hapus Tunjangan Transportasi Scale</AlertDialogTitle>
                        ...
                        <TableRow v-if="items.data.length === 0">
                            <TableCell colspan="6" class="text-center py-12 text-muted-foreground">
                                Belum ada data tunjangan transportasi scale.
                            </TableCell>
                        </TableRow>
                    </TableBody>
                </Table>
