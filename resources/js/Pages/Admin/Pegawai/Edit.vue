...
const form = useForm({
    ...
    jabatan_struktural_poin_id: p.jabatan_struktural_poin_id ?? '',
    golongan_ruang_id: p.golongan_ruang_id ?? '',
    email: p.email ?? '',
    no_hp: p.no_hp ?? '',
    alamat: p.alamat ?? '',
    ...
    strata_pendidikan: p.strata_pendidikan ?? '',
    program_mengajar: p.program_mengajar ?? '',
    ikatan_kerja: p.ikatan_kerja ?? '',
    status_kawin: p.status_kawin ?? '',
    nama_sm: p.nama_sm ?? '',
    nip_sm: p.nip_sm ?? '',
    nohp_sm: p.nohp_sm ?? '',
    pekerjaan_sm: p.pekerjaan_sm ?? '',
    nama_ibu: p.nama_ibu ?? '',
    masa_jabatan: p.masa_jabatan ?? '',
    tgl_sk_jabatan: p.tgl_sk_jabatan ?? '',
    photo: null,
    foto_sk: null,
});

function submit() {
    form.put(route(`${prefix.value}.pegawai.update`, props.pegawai.id));
}
</script>

<template>
    <Head :title="`Edit — ${pegawai.nama_pegawai}`" />

    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-semibold leading-tight text-gray-800">Edit Pegawai</h2>
                <Link :href="route(`${prefix}.pegawai.show`, pegawai.id)" class="text-sm text-gray-600 hover:underline">Kembali</Link>
            </div>
        </template>
                    ...
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <Input id="ikatan_kerja" v-model="form.ikatan_kerja" type="text" class="mt-1" />
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <div v-if="pegawai.photo" class="mb-2">
                                    <Label>Foto Saat Ini</Label>
                                    <img :src="'/storage/' + pegawai.photo" class="h-20 w-20 rounded-full object-cover" />
                                </div>
                                <Label>Ganti Foto Pegawai</Label>
                            ...
                            <div>
                                <div v-if="pegawai.foto_sk" class="mb-2">
                                    <Label>Foto SK Saat Ini</Label>
                                    <img :src="'/storage/' + pegawai.foto_sk" class="h-20 w-20 rounded object-cover" />
                                </div>
                                <Label>Ganti Foto SK</Label>
                        ...
                        <div class="flex items-center justify-end gap-3 pt-4">
                            <Link :href="route(`${prefix}.pegawai.show`, pegawai.id)" class="rounded-lg border border-input bg-background px-4 py-2 text-sm font-medium hover:bg-accent hover:text-accent-foreground">Batal</Link>
